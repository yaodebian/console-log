...
Vite 的核心原理可概括为 “基于 es module 驱动的按需编译”。

在开发环境，Vite 放弃了传统的预打包模式，转而借助浏览器对 es module 的支持，按需向浏览器提供经轻量转换后的源代码。
在热更新方面，Vite 基于 es module 的特性，仅将发生变更的模块重新编译，并通知浏览器进行模块级别的替换。
基于这些机制，Vite 能够在体量较大的项目中，仍然保持极快的冷启动与热更新。
当然，针对第三方依赖，Vite会基于 esbuild 进行预构建，同时也会对源码中的导入路径进行重写，通过这些机制，保证代码能够被浏览器正常识别。

在生产环境，Vite 选用了在 Tree Shaking、代码分割、插件生态均更加成熟的 Rollup 作为打包工具。
