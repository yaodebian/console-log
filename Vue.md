...
## Vue3 中的 keep-alive 底层实现原理是什么？

对于 keep-alive 中标记能够缓存的组件，其首次渲染时，keep-alive 会将渲染结果（VNode）按 key 存入一个 cache 的Map 中，这个 vnode 中包含完整的组件实例，包括响应式状态、生命周期状态和真实 DOM 实例。

当再次渲染该组件时，它不会重新 mount，而是从 cache 中取出对应的 VNode，将其 DOM 重新插回页面，并触发 activated，而不会触发 mounted。
...
## Vue 中的 nextTick 是做什么的？它的原理是什么？

nextTick底层是基于微任务（Promise.then 或 MutationObserver）实现的。所以它的触发是在 DOM "更新完成后、渲染前" 执行回调，基于这个时机，这个API常常用于获取到更新后的 DOM 实例、组件实例。

## computed 和 watch 的区别是什么？分别在什么场景下使用？

`computed` 和 `watch` 的核心区别在于**缓存特性**和**侧重点**不同。

**首先是 缓存机制**。`computed` 是基于它们的**响应式依赖**进行缓存的。只有当相关依赖发生改变时，它们才会重新求值。这意味着只要依赖不变，多次访问 `computed` 属性会立即返回之前的计算结果，而无需再次执行函数。相比之下，`watch` 没有缓存，数据变化即触发。

**其次是 使用场景**。`computed` 侧重于**“产出数据”**，适用于一个数据依赖多个数据计算得出的场景（一对多/多对一），且必须同步返回结果。`watch` 侧重于**“执行副作用”**（Side Effects），即在数据变化时执行与外部环境交互的操作（如发起网络请求、手动修改 DOM、操作 LocalStorage 等），它关注的是“过程”而非“返回值”。

**总结**：如果你需要一个值来描述依赖的状态，用 `computed`；如果你需要在状态变化时执行某个动作，用 `watch`。

## v-if 和 v-show 有什么区别？

`v-if` 和 `v-show` 都可以控制元素的显示与隐藏，但它们的**渲染机制**和**性能开销**完全不同。

**首先是 渲染机制**。`v-if` 是真正的**条件渲染**，当条件为假时，元素根本不会被渲染到 DOM 中（销毁/重建）。而 `v-show` 仅仅是简单的**CSS 切换**，元素始终保留在 DOM 中，只是通过 `display: none` 控制可见性。

**其次是 性能特征**。`v-if` 有较高的**切换开销**，因为涉及 DOM 的添加和移除；`v-show` 有较高的**初始渲染开销**，因为无论是否显示都要渲染。

**总结**：如果需要非常频繁地切换，请使用 `v-show`；如果在运行时条件很少改变，或者不希望初始加载不必要的内容，则使用 `v-if`。

## 为什么 Vue3 推荐使用 Pinia 替代 Vuex？

Pinia 是 Vue 官方为 Vue3 设计的新一代状态管理库，更轻量、更简单、更贴合 Composition API，并提供更好的 TypeScript 支持和 DevTools 体验。相比 Vuex，它去掉 mutation，降低心智负担，拥抱 Vue3 响应式能力，是官方推荐使用的替代方案。
